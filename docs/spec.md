...
- **A board per challenge (extends §6).** The sandbox and every challenge keep a separate board — gears, shafts, motor, and load — with its own undo history. Opening a challenge stashes the outgoing board and restores that challenge's own (empty the first time), so a train never carries into a brief whose palette forbids its sizes. Boards persist in `localStorage` (`gtw.boards.v1`). The GIVEN block of every challenge states the attach step: motor to the input gear, load to the output gear.

- **The train stays on the sheet (extends §4).** Gear positions are canvas pixels, so when the drawing area shrinks (window resize, challenge panel opening) or a board is restored on a smaller screen, the whole layout is shifted back onto the sheet as one rigid piece so every mesh survives. A train too large for the sheet is pinned to the top-left; each hub still beyond the edge gets an arrow marker on the sheet edge and is counted in the readout, so a gear that still counts toward a challenge is never silently invisible.
...
- **UI-logic test layer (extends §9).** Beyond the solver's `test.js`/`verify-html.js`, the non-canvas UI decisions are factored into a second pure module, `dev/logic.js` (download naming, palette limits, snap/grab geometry, progress serialize/merge, the reveal-gate state machine, "show the working" stage reconstruction). `dev/test-logic.js` unit-tests it and `dev/verify-logic.js` confirms the verbatim inline copy in `index.html` stays in sync. `dev/e2e.js` adds in-browser checks (headless Chromium via Playwright, an optional dev-only dependency) for sheet fitting, per-challenge boards, and the print report; pointer dragging remains manual.
